...
def addOrUpdateCourse(r: requests.Session, userid:str, orgid:str, duration:int, yid: int):
    ...
    payload = {"studyTime": duration + 100,"classId":yid,"userId":userid,"areaId1":orgid[:5],"areaId2":orgid[:9],"areaId3":orgid[:13],"areaId4":orgid,"status":1,"retakes":0}
    req = r.post(url, data=json.dumps(payload))
    if req.status_code == 200:
        return True
    else:
...
if __name__ == '__main__':
    ...
    for userData in userlist:
        ...
        for item in retakesList(session):
            step1 = learningRecords(session, openid=userData, yid=item['i'])
            time.sleep(1)
            step2 = addOrUpdateCourse(session, userid=userlist[userData]['userid'], orgid=str(userlist[userData]['nid']), yid=item['i'], duration=item['d'])
            time.sleep(1)
